...
const DailySalesByMedia = () => {

  const { data: dailySalesByMedia } = useQuery(
    'dashboard/daily-sales-by-media',
    dashboardApi.getDailySalesByMedia,
    { refetchInterval: 10000, initialData: [] }
  )

  return (
    <Paper>
      <Text size="xl" weight="600" color="gray_900">Vendas diárias por mídia({new Date().getDate()}/06)</Text>
      
      <Divider my={10} />
      
      <Flex gap={20} style={{ overflowX: 'auto' }}>

        {dailySalesByMedia.map(sale => (
          <Paper key={`daily-sales-by-media-${sale.id}`} color="indigo_500" style={{ minWidth: '250px' }}>
...
export default DailySalesByMedia